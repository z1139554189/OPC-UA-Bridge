...
### 2026-06-19 DI趋势图 + 报告生成路径修复

| # | 问题 | 根因 | 教训 |
|---|------|------|------|
| 56 | AI 多次在错误的 workspace 目录操作，而非正确项目目录 | Ask 模式 workspace 路径和项目目录不同，AI 未确认当前工作目录 | **执行任何文件操作前，先用 `pwd` 或绝对路径确认当前目录** |
| 57 | DI 特征趋势曲线显示 230 个窗口（全量），而非仅分析期 | `all_window_vals` 数据源用了 `window_features`（全量），而非 `recent_windows`（分析期） | **分析期相关的数据序列必须明确来源是 `recent_windows`，不能混用 `window_features`** |
| 58 | 报告生成到 `data/` 目录而非 `reports/` | AI 手动指定了 `--output data/...`，覆盖了脚本默认的 `REPORT_DIR` | **生成报告时不加 `--output` 参数，让脚本自动放到 `reports/` 目录** |
...
- **基线/分析期逻辑**（2026-06-19 最终版）：
  - 基线期 = 前7天稳态数据的特征中位数（固定）
  - 分析期 = 第8天~最新数据（**自适应长度**，可能是3天/5天/7天/30天）
  - 数据 <7天 → 返回 `data_insufficient: True`，报告显示"需积累7天以上稳态数据"
  - 变化% = (分析期中位数 - 基线中位数) / |基线中位数| × 100%
  - DI趋势曲线（`values` 字段）= 仅分析期窗口值序列（不含基线期）
- **两道门槛串联**：先检查 `eng_meaningful`（绝对变化 ≥ `ENG_MIN_TOTAL_CHANGE`），通过后才根据 `change_pct` 判断退化等级
- **工程意义门槛**（`ENG_MIN_TOTAL_CHANGE`）：均值/RMS ≥0.3A，标准差 ≥0.05A，偏度 ≥0.15，etc.
- **报告输出**：HTML（Chart.js 内嵌，不依赖外部 CDN），自动生成到 `reports/` 目录，文件名含时间戳
